...
export class ClassServiceListComponent implements OnInit {
	...
	private loadCurrentUserAndEstablishment(): void {
		...
		this.toastService.show('Loading user information...', 'info');
	...
	loadClassServices(): void {
		...
		this.loading = true;
		this.toastService.show('Loading class services...', 'info');
		...
		this.classServiceService.getClassServicesByEstablishment(this.currentEstablishmentId).subscribe({
			next: (classes) => {
				...
				this.loading = false;
				this.forceStopLoading();
				this.toastService.show(`${classes.length} class services loaded`, 'success');
			},
			error: (err) => {
	...
	private handleError(context: string, error: unknown): void {
		...
		console.error(`${context}:`, error);
		this.toastService.show(`${context}: ${errorMessage}`, 'danger');
	}
